...
export interface DesktopGuestSessionMountService {
  ...
  refresh(mountId: string): Promise<void>;
  recordConnectionFailure(mountId: string, error: Error): Promise<void>;
  importInvitation(
    code: string,
...
export function createDesktopGuestSessionMountService(input: {
  ...
  const refreshOnce = async (mountId: string): Promise<void> => {
    ...
    // admitted, shutdown drains this read before closing its Runtime Host.
    while (true) {
    ...
    const session = await input.getSharedSession(mountId);
    if (removingMounts.has(mountId) || invalidatedAccessMounts.has(mountId)) return;
    if (!session) {
  ...
  return {
    ...
    async recordConnectionFailure(mountId, error) {
      if (isRejectedAccessFailure(error)) await clearSessionProjection(mountId);
    },
