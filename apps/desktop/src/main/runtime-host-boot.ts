...
const startLocalRuntimeHostManager = () => startRuntimeHostDesktopManager(
  ...
  },
  {
    ...
    onTargetStateChanged: (state) => {
      ...
      if (profileAccess === 'session_guest') notifyGuestSessionMountsChanged();
      if (profileAccess === 'session_guest' && state.readiness === 'ready') {
        void guestSessionMountService.refresh(state.target.profile.id).catch((error: unknown) =>
          console.warn('[runtime-host] shared Session projection refresh failed:', error),
        );
      }
      if (profileAccess === 'session_guest' && state.readiness === 'unavailable') {
        void guestSessionMountService
          .recordConnectionFailure(state.target.profile.id, state.error)
          .catch((error: unknown) =>
            console.warn('[runtime-host] shared Session invalidation failed:', error),
          );
      }
